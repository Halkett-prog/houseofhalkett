...
export default function CalculatorComponent() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      // Add to Cart function that actually saves data
      window.addToCart = function() {
        console.log('Add to Cart clicked!');
        
        // Gather all the calculator data
        const config = {
          ...
          coverageHeight: document.getElementById('coverageHeight')?.value || '',
          ...
          trimMaterial: document.getElementById('allTrimMaterial')?.value || '',
          ceilingTrim: document.getElementById('ceilingTrim')?.checked || false,
          baseTrim: document.getElementById('baseTrimFull')?.checked || document.getElementById('baseTrimWainscot')?.checked || false,
          wallCount: document.getElementById('wallCount')?.value || '',
          results: document.getElementById('resultsContainer')?.innerHTML || '',
          cost: document.getElementById('costEstimateContainer')?.textContent || ''
        };
      ...
      // Other functions that need to be available
      ...
      window.generatePDF = function() {
        ...
        window.print(); // For now, just print
      };
      
      window.emailSpecification = function() {
        console.log('Email specification');
        window.location.href = 'mailto:?subject=HALKETT Wall Specification';
      };
      
      window.showSampleRequest = function() {
      ...
      window.closeGalleryOutside = function(event) {
      ...
      window.submitSampleRequest = function() {
        console.log('Sample request submitted');
        alert('Sample request submitted! We will contact you soon.');
        window.closeSampleModal();
      };
      
      window.handleCoverageChange = function(target) {
        const wainscotHeight = document.getElementById('wainscotHeight');
        if (target.value === 'partial') {
          wainscotHeight.style.display = 'block';
        } else {
          wainscotHeight.style.display = 'none';
        }
      };
      
      window.updateTrimOptions = function() {
        console.log('Update trim options');
      };
      
      window.updateBoardRecommendation = function() {
        console.log('Update board recommendation');
      };
      ...
      window.showRecentConfigurations = function() {
      ...
      // Make sure Step 1 is active on load
      document.getElementById('step1')?.classList.add('active');
    }
  }, []);
